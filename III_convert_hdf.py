...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 13:36:37 2017
...
def _find_all_houses(input_path):
    ...
    return _matching_ints(dir_names, '^house_(\d\d?)$')
...
def _load_csv(filename, columns=[('power', 'ac_type')], tz= 'Asia/Taipei'):
    """
    Parameters
    ----------
    filename : str
    ...
    df = pd.read_csv(filename, sep=',', names=columns,
                     dtype={m:np.float64 for m in columns})
...
def parse_args():
    global data_type # III (eight household), III_Commercial, or HENS_PongHu
    parser = argparse.ArgumentParser()
    optional_named_arguments = parser.add_argument_group('optional named arguments')
    optional_named_arguments.add_argument('-d', '--data_name',
                                          help='data type: III (eight household), III_Commercial, or HEMS_PongHu',
                                          required=True)
...
def set_log_level():
    ...
    warnings.filterwarnings('ignore', message="RuntimeWarning: numpy.dtype size changed, may indicate binary incompatibility.")

if __name__ == '__main__':
    parse_args()
    if data_type == 'III':
        REDD = os.path.join('data','III') + '/'
        Out = os.path.join('data', 'III_' + strftime('%Y_%m_%d') + '.h5' )
    elif data_type == 'III_Commercial':
        REDD = os.path.join('data','III_Commercial') + '/'
        Out = os.path.join('data', 'III_Commercial', 'III_Comm_'+ strftime('%Y_%m_%d') + '.h5' )
    elif data_type == 'HEMS_PongHu':
        REDD = os.path.join('data','HEMS','PongHu') + '/'
        Out = os.path.join('data', 'HEMS_PongHu_'+ strftime('%Y_%m_%d') + '.h5' )       
    else:
        print("Non vaild data type !")

    convert_redd(redd_path = REDD, output_filename = Out, format='HDF')
